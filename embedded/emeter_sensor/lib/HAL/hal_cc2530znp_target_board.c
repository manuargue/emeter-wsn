...
/** Function pointer for the ISR called when timer B generates an interrupt*/
void (*timerBIsr)(void);
...
signed int blinkLedsTwice(unsigned int ms)
{
    setLed(0); setLed(1);
    delayMs(ms);
    clearLeds();
...
{  
    // modificar esta constante si se cambia el divisor del clock
    // o: unsigned char maxSeconds = 0xFFFF / vloFrequency;
    ...
    // CCR0 max 2^16. con vlo=12176 Hz, seconds < 5.38
    // con divider (ID_x), seconds < 10.76 (/2), 21.52 (/4), 43.05 (/8)
    CCR0 = vloFrequency * (seconds);          // generate int
    TACTL = TASSEL_1 + MC_1;// + ID_x;        // ACLK, upmode, divider ACLK/x
    return 0;
}
